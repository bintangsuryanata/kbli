<?php
    ...
    $tiket2_id = isset($_GET['tiket2_id']) ? $_GET['tiket2_id'] : false;
...
<?php	
		...
		$query = mysqli_query($koneksi, "SELECT * FROM tiket2 WHERE tiket2_id='$tiket2_id' AND status='on'");
		$row = mysqli_fetch_assoc($query);
?>
...
<div class="lg:mt-12 sm:mb-24 sm:mx-auto sm:max-w-7xl">
        <div class="text-black mx-2 mb-4 mt-14 sm:mx-5 hover:text-black text-start sm:px-2 py-2 rounded-xl bg-[#FFF] text-base shadow-lg transition-all duration-300 shadow-slate-500"
            ...
            <div class="sm:mb-4.5 sm:-mt-[44px] ml-[31px] mt-8 mb-5 ">
                    ...
                    <p class="bg-[#525252] flex justify-center items-center p-2 mr-3 text-[14px] text-white w-[90px] h-[50px] rounded-sm text-center -mt-[50px] ml-[110px] text-[12px]  font-black"><?php echo $row[ 'kode_2' ]?></p>
            </div>
            ...
            <p class="text-[15px] font-light  ml-8 mt-3 "><?php echo $row[ 'nama_tiket_2' ]?></p>
            ...
            <?php
                     if ($tiket2_id) {
                        $queryTiket = mysqli_query($koneksi, "SELECT * FROM tiket3 WHERE tiket2_id='$tiket2_id' AND status='on' ORDER BY kode_3 ASC");
                    } else {
