<?php
    ...
    $tiket3_id = isset($_GET['tiket3_id']) ? $_GET['tiket3_id'] : false;
...
<?php	
		...
		$query = mysqli_query($koneksi, "SELECT * FROM tiket3 WHERE tiket3_id='$tiket3_id' AND status='on'");
		$row = mysqli_fetch_assoc($query);
?>
...
<div class="lg:mt-12 sm:mb-24 sm:mx-auto sm:max-w-7xl">
        <div class="text-black mx-2 mb-4 mt-14 sm:mx-5 hover:text-black text-start sm:px-2 py-2 rounded-xl bg-[#FFF] text-base shadow-lg transition-all duration-300 shadow-slate-500"
            ...
            <div class="sm:mb-4.5 sm:-mt-[44px] ml-[31px] mt-8 mb-5 ">
                    ...
                    <p class="bg-[#525252] flex justify-center items-center p-2 mr-3 text-[14px] text-white w-[90px] h-[50px] rounded-sm text-center -mt-[50px] ml-[110px] text-[12px]  font-black"><?php echo $row[ 'kode_3' ]?></p>
            </div>
            ...
            <p class="text-[15px] font-light  ml-8 mt-3 "><?php echo $row[ 'nama_tiket_3' ]?></p>
            ...
            <?php
                     if ($tiket3_id) {
                        $queryTiket = mysqli_query($koneksi, "SELECT * FROM tiket4 WHERE tiket3_id='$tiket3_id' AND status='on' ORDER BY kode_4 ASC");
                    } else {
                        $queryTiket = mysqli_query($koneksi, "SELECT * FROM tiket4 WHERE status='on' ORDER BY kode_4 ASC");
                    }
                ...
                echo "
                    <a href='" . BASE_URL . "index.php?page=main-4&tiket4_id=$row[tiket4_id]'>
                        <div class='text-black border border-black  mt-3 hover:text-black text-start px-2 py-2 rounded-lg shadow-slate-500 mb-8 mx-6'
                        data-aos='fade-left' data-aos-duration='1500'>
                           <div class='relative mb-2.5'>
                                <p class='bg-[#D4D4D4] flex justify-center items-center font-black p-2 mr-3 text-[14px] text-black w-10 rounded-lg text-center'>$row[kode_4]</p>
                                <p class='-mt-7 ml-[60px] text-[15px] font-black leading-5 text-[#252525]'>$row[nama_tiket_4]</p>
                           </div>
                        </div>
                    </a>";
                    }
